#!/usr/bin/env python3
"""
Migration: Remove MAX_TOTAL_SCORE column from daily_anomaly_snapshot table

This migration removes the max_total_score column that was added in the previous migration.
"""
...
def down():
    """Re-add MAX_TOTAL_SCORE column to daily_anomaly_snapshot table."""
    conn = db.connect()
    try:
        with conn.cursor() as cur:
            # Re-add MAX_TOTAL_SCORE column
            cur.execute("""
                ALTER TABLE daily_anomaly_snapshot 
            ...
            print("✓ Re-added MAX_TOTAL_SCORE column to daily_anomaly_snapshot table")
